...
onMounted(() => {
  ...
  script.onload = () => {
    ...
    window.L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
        maxZoom: 18,
        attribution: '© OpenStreetMap | 九机数据分析演示 (前端模拟)'
    }).addTo(mapInstance)
    ...
    const dotIcon = window.L.divIcon({
        ...
        html: `<div style="background:#ff4757; width:10px; height:10px; border-radius:50%; border:2px solid #fff; box-shadow:0 0 6px rgba(255,71,87,0.8);"></div>`,
...
.leaflet-popup-content-wrapper { border-radius: 8px; box-shadow: 0 8px 25px rgba(0,0,0,0.15); padding: 0; overflow: hidden; }
